        ...
        if neighbors is None:
            ...
            while open_set:
                ...
                if self.is_goal_reached(current_node[1], current_node[0]):
                    return parent_nodes, self.generate_path(explored, current_node[0], current_node[1], current_node[2])
    ...
    while selected_box is None:
        for event in pygame.event.get():
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                pygame.display.update()
                second_point = None
                while second_point is None:
                    for event in pygame.event.get():
                        if event.type == pygame.MOUSEBUTTONDOWN:
                            mouse_x, mouse_y = pygame.mouse.get_pos()
                            second_point = (mouse_x, mouse_y)
                            delta_x = second_point[0] - selected_box[0]
                            delta_y = selected_box[1] - second_point[1]
                            ...
                            if length == 0:
                                ...
                                scale = 50 / length
                                end_x = selected_box[0] + delta_x * scale
                                end_y = selected_box[1] - delta_y * scale
                            pygame.draw.line(screen, RED, selected_box, (end_x, end_y), 2)
                            angle = math.atan2(-delta_y, delta_x) * 180 / math.pi
                            pygame.display.update()
                            return selected_box, angle
...
def main():
    while True:
        screen = draw_shapes()
        start_node, st_orientation = select_box(screen)
        end_node, end_orientation = select_box(screen)
        if st_orientation < 0:
